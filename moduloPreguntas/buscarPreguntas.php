...
try {
    ...
    // Si no hay término de búsqueda, cargar todas las preguntas
    if (empty($searchTerm)) {
        include 'cargarPreguntas.php';
        return;
    }
    ...
    $query = "SELECT p.id_pregunta, p.texto_pregunta, p.puntos, p.activa, 
             ...
             WHERE p.texto_pregunta LIKE :search 
                OR c.nombre_categoria LIKE :search 
                OR t.nombre_tema LIKE :search
                OR r.texto_respuesta LIKE :search
    ...
    if (count($preguntas) > 0) {
        ...
        echo "<tr><td colspan='7' class='text-center'>No se encontraron preguntas que coincidan con '{$searchTerm}'</td></tr>";
    }

} catch (Exception $e) {
    echo "<tr><td colspan='7' class='text-center text-danger'>Error en la búsqueda: " . htmlspecialchars($e->getMessage()) . "</td></tr>";
}
?>
